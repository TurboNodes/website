    ...
    <Card className="bg-gray-900 border-gray-800 p-4 h-full">
      ...
      <div className="space-y-3">
        {nodes.map((node, index) => (
          <div key={node.id} className={`border ${node.isActive && node.isConnected ? "bg-green-500/10 border-green-500/30"
              : "bg-red-500/10 border-red-500/30"} border-gray-800 rounded-lg p-3 bg-gray-900/50`}>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full ${node.isActive && node.isConnected ? 'bg-green-500' : 'bg-red-500'}`} />
                <span className="text-sm font-medium text-white">Node #{index + 1}</span>
              </div>
              <Badge 
                variant={node.isActive && node.isConnected ? "default" : "destructive"}
                className={node.isActive && node.isConnected ? "bg-green-500/20 text-green-500 border-green-500/30" : "bg-red-500/20 text-red-500 border-red-500/30"}
              >
                {node.isActive && node.isConnected ? 'Online' : 'Offline'}
              </Badge>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              ...
              <div className="text-gray-400">
                <span className="text-orange-500 font-medium">{node.bandwidthUsed?.toFixed(2) || '0.00'}</span> GB
              </div>
              <div className="text-gray-400">
